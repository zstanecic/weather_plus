...
# USO: #
* Pressione NVDA + w Para ver a temperatura corrente e as condições atmosféricas.
* Pressione NVDA + shift + W para obter as condições nas próximas 24 horas e nos próximos 9 dias.
* Pressione NVDA + shift + ctrl + w Para definir uma cidade temporária.
* Pressione NVDA + shift + control + alt + w para abrir o diálogo de configurações do Weather Plus.
* Pressione NVDA + alt + w para conhecer a última actualização do relatório das condições atmosféricas.
* Pressione control + shift + w para alternar entre Fahrenheit, Celsius ou Kelvin.

# Para configurar o Weather Plus: #
Leia a seção: Primeiras Configurações do Weather Plus.
...
Vá para o menu do NVDA:

Submenu preferências

Submenu Opções do Weather Plus

Mostrar itens de configuração

# Pressione o item para: #

* Definir e gerenciar as suas cidades...
	* Mostra ou permite definir as cidades a partir de  uma lista
* Documentação
	* Abre o ficheiro de ajuda para o idioma actual
* Procurar por actualizações...
	* Notificar-me, se houver uma nova versão disponível.
...
A menssagem a seguir é mostrada apenas na primeira vez!

Configurações Predefinir

Nenhum

F1: ajuda na localização, F2: selecção do último tab, F3: lista e caixa de edição, F4: controlo da duração da previsão do tempo, F5: controlos do volume.

Introduza uma cidade, woeId ou escolha uma da lista, se disponível.

Nota: A tecla F5 está disponível se os efeitos sonoros estiverem activados.

Depois de pressionar o item "Definir e gerenciar as suas cidades...", encontrará outros botões como mostrado abaixo:

# Testar #

Testa a validade do woeID da cidade entrada e encontra o nome da cidade correspondente, ou vice-versa.

# Adicionar #

Adiciona a cidade actual à sua lista.

Este botão é activado se seleccionar uma cidade da lista, ou quando um código passou no teste de validação.

# Detalhes #

Exibe informações sobre a cidade atual.
...
Este botão é activado se os efeitos sonoros estiverem instalados e activados, e seleccionar uma cidade da lista, ou quando um código passou no teste de validade.

# Predefinir #

Predefine a cidade actual e usa-a sempre que o extra for reiniciado.

Este botão será activado se seleccionar uma cidade previamente adicionada à lista, mas não predefinida, ou se o seu código passou no teste.

# Remover #

Apaga a cidade actual da sua lista.

Este botão é activado se seleccionar uma cidade previamente inserida na lista.

# Renomear #

Renomeia a cidade actual.

Este botão é activado se seleccionar uma cidade previamente inserida na lista.

# Importar novas cidades... #

Permite incorporar, na sua lista, novas cidades, importando-as de outra lista com a extensão *.zipcodes; pode seleccionar a cidade que quer importar, marcando a caixa de marcação associada.

# Exportar suas cidades... #

Permite guardar a sua lista de cidades numa pasta específica.

Este botão é activado se adicionou e guardou, pelo menos, uma cidade na lista.

# Escala de medição de temperatura: #

Use os botões de opção para escolher entre:

* Centígrados (por padrão)
* Fahrenheit
* Kelvin

# Mostrar os graus como: #

Use os botões de opção para escolher entre:

* Centígrados `-` Fahrenheit `-` Kelvin (por padrão)
* C `-` F `-` K
* Não especifique

Caixa de combinação:

# Previsão do tempo acima de dois dias: 3 #

Escolha de 1 a 10; (3 dias (por padrão)

Alternar a caixa de selecção para:

# Copiar o boletim meteorológico e previsão do tempo, incluindo detalhes da cidade, para a área de transferência #

caixa de selecção não marcada (por padrão)

# Activar efeitos de áudio (Apenas para as condições atmosféricas actuais) #

Esta caixa de verificação também permite gerir a instalação dos efeitos de som;

Se os efeitos sonoros estiverem instalados e a caixa de selecção estiver activada, a tecla F5 e a configuração de volume estarão disponíveis.

Também estará disponível uma caixa de selecção adicional:

* Usar apenas efeitos climáticos.

Ppode alterar o volume geral ou modificar o último efeito de som ouvido e filtrar os outros sons no seu ambiente.
...
Se estiver activado, permite ouvir apenas efeitos climáticos como chuva, vento, trovão, etc., filtrando todos os ambientais.

Caixa de marcação não marcada (por padrão)

# Activar a leitura das horas em formato de 24 horas #

Se desmarcada anuncia o tempo em formato de 12 horas, por exemplo: 00:00 `-` 12:00.

caixa de selecção marcada (por padrão)

# Indica esperando com um sinal sonoro ao conferir o último boletim #

Se marcada, ouvirá um sinal sonoro para indicar o estado das pesquisas em andamento até encontrar o último boletim meteorológico mais actualizado.

caixa de selecção marcada (por padrão)

# Activar botões de ajuda na janela de configurações #

caixa de selecção marcada (por padrão)

# lê informações sobre o vento #

caixa desmarcada )por padrão)

Se estiver activada, também pode marcar:

* Adicionar direcção do vento;

Indica a direcção de que sopra o vento.

caixa marcada (por padrão)

* Adicionar velocidade do vento;

Indica a velocidade, em quilómetros ou milhas por hora.

caixa marcada (por defeito)

* Adicionar velocidade do vento, em metros por segundo;
...
# Ler informações atmosféricas; #

caixa desmarcada (por padrão)

se estiver activada, pode marcar também:

* Adicionar valor da humidade;

indica a percentagem de humidade.
...
Se estiver marcada, activa uma caixa de marcação adicional, que lhe permite indicar a pressão atmosférica em milímetros de mercúrio.
...
# Use a vírgula para separar as décimas#

se estiver activada, usa a vírgula como separador decimal, de outro modo, usa o ponto.

Caixa desmarcada, (por padrão)

# Procurar por actualizações #

Se estiver activada, diz-lhe quando há actualizações do addon.

caixa de selecção marcada (por padrão)

# Pressione o botão "OK", para confirmar a acção, ou o botão "Cancelar", para cancelar a acção. #

Se tiver modificado a lista de cidades, quando pressionar o botão "cancelar", será alertado e ainda pode salvá-la.

# Nota: as suas configurações serão guardadas em  ficheiros chamados: #

* "Weather.ini": configurações iniciais do Weather Plus.
* "Weather.volumes": os níveis personalizados de volume de áudio, independentemente do volume global.
* "Weather.zipcodes": lista de cidades com o seu código de área e definições.
...
* Praticamente todos os recursos da versão anterior 4.8 estão de volta e mantém o botão "Renomear".
* Agora compatível também com o Python 3.

# Versão 5.0.1 #
* Corrigido bug, que retornava uma string vazia se a velocidade do vento em mph fosse 0.
* Corrigido bug que causou efeitos sonoros não consistentes com o fuso horário a ser reproduzido.
* Ajustou o número de dias de previsão de 9 para 6 em readme.

# Versão 5.0 #
* Agora, O Weather Plus usa a API do APIXU, a qual, em minha opinião,  é melhor que a anterior.
...
# Versão 4.8 #
`#`Alterações na janela de configurações do Weather Plus :
...
# Versão 4.7.7 #
* Removida uma notificação desnecessária de download completo quando se faz a atualização do Weather Plus.
...
# Versão 4.7.5 #
* O comando MenuItem.GetLabel () está obsoleto e foi substituído por MenuItem.GetItemLabelText ().
...
* Foi removido o seletor de arquivos na seção de atualização; Agora, o arquivo de atualização é carregado para a pasta temporária, resolvendo problemas devido a usuários não especializados.

# Versão 4.6.9 #
* Foi adicionada atradução árabe, graças à colaboração de Wafik Immaculate).

# Versão 4.6.8 #
* Atualização da tradução para a versão portuguesa e para a versão brasileira (por a Alberto Mendonça).
...
# Versão 4.6.4 #
* Melhorada a leitura da actual hora local; As chaves de procura tornaram-se mais precisas.

# Versão 4.6.2 #
* Corrigido bug: após uma verificação de atualizações, o menu "definir uma cidade temporária ..." era ativado mesmo que não existisse lista de cidades disponíveis.
* Corrigido bug; incapaz de configurar o WP quando o weather.ini ainda não tivesse sido criado.
...
*Melhorada a prevenção da abertura simultânea de janelas do programa: quando ocorra, a primeira janela mantém-se como fundo.
...
* Foram corrigidas a localização, as configurações e a documentação em alemão.

`#`Mudanças na janela de configuração do Weather Plus:

* Foi adicionada uma nova caixa de marcação;
	* Pode activar a vírgula como separador decimal, de outro modo, o ponto continuará a ser o separador padrão.
...
# Versão 4.5 #
* Adicionada a tecla de atalho NVDA+shift+control+alt+w;
	* Abre o diálogo de configurações do Weather Plus.
* Foram corrigidas algumas strings em inglês.
...
# Versão 4.4 #
* Corrigido erro na leitura da nova seqüência de versão, durante um tempo limite de conexão.
...
# Versão 4.3.2 #
* Foi adicionada a tradução em romeno, graças à colaboração de Florian Ionașcu).
...
# Versão 4.2.3 #
* Agora O Weather Plus pode executar algumas tentativas de ligação antes
de avisar o usuário do mau funcionamento da "WoeId" que está a ser
usada, emitindo um "beep" por cada tentativa feita. Se o desejar, este
"beep" pode ser desabilitado através da marcação de uma caixa, nas
...
# Versão 4.0 #
* Modificadas algumas partes do código, alterando as instruções constantes da função eval().
...
# Versão 3.9.5 #
* Adicionadas duas novas sequências de caracteres para a lista de serviços meteorológicos.
* corrigidos 2 bugs.
...
# Versão 3.7 #
* Adicionada a possibilidade de desactivar a opção "metros por segundo", aplicada ao vento;
* Adicionada a possibilidade de usar a unidade de medida em milhas por polegadas quadradas.
* Fixados 2 bugs.
...
* Adicionados controlos para WoeId and Zip Code inválidos, encontrados na rede.
	* Há indicação de códigos que deixaram de funcionar. Nestes casos, agradecia que me fossem comunicados, para que os retire da lista.
...
* Adicionada a tradução para Português europeu(Agradecimentos a Ângelo Miguel Abrantes).
...
# Versão 3.1 #
* Adicionada a tradução para sérvio (graças a amável cooperação de Gašić Dejan * Gashich Deyan).
...
# Versão 3.0 #
* Adicionada tradução para eslovaco (graças a amável cooperação de Vitek Jirasek).
* Adicionadas traduções para Português do Brasil e de Portugal (graças a amável cooperação de Adair Knaesel).
...
* Adicionado comando insert+alt+w, o qual informa a última atualização do boletim meteorológico.
* Adicionado scriptCategory que faz com que as teclas de atalho sejam correctas.
...
# Versão 2.9 #
* Adicionada nova opção ao menu "importar", a qual permite  selecionar o conteúdo do arquivo a ser importado.
...
# Versão 2.8 #
* Corrigido bug em "detalhes"
	* Abre a janela de ocorrências quando não encontra a cidade.
...
* ADicionaddas 3 caixas de verificação para gerenciar a informação listada anteriormente.
...
# Versão 2.5 #
* Adicionado um comando para alterar temporariamente a escala de temperatura entre Celsius ou Fahrenheit, este comando tambén é efetivo na janela de configuração.
* corrigido um erro, se o usuário não pressionou os botões "Adicionar" ou "Predefinir", não permite pronunciar o nome da cidade, já que não estava incluído na lista.
* Adicionadas novas sequências na lista dos boletins meteorológicos.

`#`Mudanças na janela de configuração do Weather Plus:

* Adicionado botão para abrir uma página web de busca com o fim de comprovar os códigos de área de todo o mundo.
* Adicionada a possibilidade de importar/exportar os códigos de área dos seus amigos.
* Adicionada a possibilidade de copiar o boletin meteorológico  ou a previsão o tempo para a área de transferência.
* Adicionada a possibilidade de ouvir efeitos sonoros meteorológicos, a opção tambén permite a instalação/atualização dos efeitos de áudio.
* Adicionados botões de ajuda no gerenciamento de códigos de área.
* Mudança no modo de visualização da janela, os menus do NVDA são disponibilizados quando estão abertos.

# Versão 2.4.4 #
* Adição de duas sequências na lista dos boletins meteorológicos
* Adição de localização Espanhol e Francês (graças a Pablo Vargas e Rémy Ruiz)

# Versão 2.4.3 #
* Adicionada a previsão meteorológica para os próximos 4 días
* Adição de uma sequência na lista dos boletins meteorológicos
* A lista de códigos de área temporários fica agora ordenada depois de cada nova inserção

# Versão 2.4 #
* Foi corrigido um erro que impedía Guardar e gerenciar corretamente os nomes das cidades que contêm vogais acentuadas.

# Versão 2.3 #
* Removida a caixa de diálogo para estabelecer a escala de medição de temperatura tendo sido adicionada uma nova interface gráfica do usuário que lhe permite configurar tudo en uma só janela.
* Em seguida, tambén é possível testar / adicionar / remover / predefinir o Código de Área ppadrão estabelecido em uma lista.
* Modificada a caixa de diálogo para introduzir, por escrito, um código de área, agora, no modo temporário, permite-lhe estabelecer um Código de Área previamente adicionado na lista   no menú Preferências.
* Agora a documentação pode ser lida em Inglês, se a configuração do idioma não estiver incluída nos documentos.

# Versão 2.2 #
* Solucionado um problema que não lhe permitía abrir a documentação das versões estáveis do NVDA

# Versão 2.1 #
* Foi corrigido um erro que não fechava corretamente o addon, o que impedía a atualização do ícone do NVDA na bandeja do sistema.

# Versão 2.0 #
* O menu de configuração do Weather Plus    foi movido para  o submenu Preferências
...
* adicionado ao weather.ini a configuração de leitura / escrita, agora já não terá que editar o arquivo de origen.
