...
# Použitie: #
* Stlačte NVDA + w na zistenie informácií o aktuálnej teplote a poveternostných podmienkah.
* Stlačte NVDA + shift + W na zistenie predpovede počasia na 24 hodín až   9 dní.
* Stlačte NVDA + shift + ctrl + w na nastavenie dočasného mesta.
* Stlačte NVDA + shift + control + alt + w na otvorenie dialógu nastavenia doplnku Weather Plus.
* Stlačte NVDA + alt + w na získanie informácie o poslednej aktualizácii predpovede počasia.
* Stlačte control + shift + w na prepínanie medzi stupňami Fahrenheita, Celsia alebo Kelvina.

# Nastavenie Weather Plus: #
Prečítajte si oddiel: Weather Plus Prvotné Nastavenia.

--------------------------------------------------------------------------------

# Weather Plus Prvotné Nastavenia: #

Teraz si musíte pred prvým použitím  addon Weather Plus nastaviť!

Prejdite do:

Podponuky možnosti

Podponuky Nastavenie doplnku Počasie+
...
* Nastaviť a ovládať kód mesta
	* Zobrazí sa alebo umožní nastaviť kód mesta zo zoznamu
* Nápoveda
	* Otvorí nápovedu pre aktuálne zvolený jazyk
* Skontrolovať aktualizácie...
	* Upozorniť, ak je k dispozícii aktualizácia

Stlačte tlačidlo

# Nastaviť a ovládať kód mesta #

Zobrazí alebo umožní nastaviť kód mesta zo zoznamu

Len po prvom spustení  je zobrazená následujúca správa!

Nastavenia Predvolené mesto

Nič

F1: nápoveda o umiestnení, F2: posledný výber TAB, F3: zoznam a edit pole, F4: kontrola dĺžky predpovede počasia, F5: ovládanie hlasitosti.

zadajte mesto, kód mesta podľa WOEID  alebo ho vyberte zo zoznamu, ak je dostupné.

Poznámka: Klávesa  F5 je dostupná, iba ak sú aktivované zvukové efekty.

Po stlačení tlačidla "Nastaviť a ovládať kód mesta" môžete prechádzať nasledujúcimi tlačidlami:

# Testovanie #

Slúži na testovanie platnosti kódu daného mesta, a nachádza k nemu priradený názov.

# Pridať #

Pridá aktuálny kód mesta do zoznamu.

tlačidlo je aktívne v prípade výberu mesta zo zoznamu alebo po úspešnom testovaní vyhľadania mesta.

# Podrobnosti o meste #

Zobrazuje informácie o aktuálne vybratom meste.
...
Prednastaví kód mesta ako predvolený, a bude vždy východzí po reštarte.

Tlačidlo je aktívne v prípade vybratého mesta, ktoré ste predtým pridali a uložili do zoznamu predvolených, alebo po úspešnom testovaní vyhľadania mesta.
...
# Importovať nové kódy miest a krajín ... #

Umožňuje zahrnúť do zoznamu nové mestá, ktoré sú importované z iného zoznamu z rozšírenia * .zipcodes; môžete si vybrať tiež mestá cez príslušné začiarkavacie políčko.

# Exportovať vlastné kódy miest a krajín... #

Uloží zoznam miest do zvoleného umiestnenia

tlačidlo je aktívne v prípade pridania a uloženia aspoň jedného mesta do zoznamu.

# Jednotky merania teploty: #

Vyberte si medzi jednotkami pomocou prepínača
...
# Nastaviť pomocou prepínača #

* Celzius `-` Fahrenheit `-` Kelvin (východzie)
* C `-` F `-` K
* Neurčovať

Zoznamový rámik:

# Predpoveď na dnešok je: 3 #

Vyberte si medzi 1 až 9 (3 dni je predvolené)

Začiarknite pre:

# Kopírovať správu o počasí vr. predpovede a podrobností o meste do schránky #
...
# Zapnúť ohlasovanie v 24 hodinovom formáte #

Ak nie je začiarknuté, ohlasuje čas v 12 hodinovom formáte napr.: 12 AM `-` 12 PM

vo východzom stave začiarknuté

# Pípaním indikovať čakanie pokiaľ sa hľadá posledná správa #

Ak je zapnuté, počujete pípanie  indikujúce  úroveň priebehu skúmania až po nájdenie poslednej aktualizovanej správy o počasí .
...
Ak je dostupné, môžete ho aktivovať:

* Pridaná informácia o smere vetra;

Indikuje pôvod vetra.

vo východzom stave začiarknuté)

* Pridaná informácia o rýchlosti vetra;

Indikuje  rýchlosť vetra v kilometroch alebo míľach za hodinu.

vo východzom stave začiarknuté

* Pridaná rýchlosť vetra v metroch za sekundu ;

vo východzom stave začiarknuté

* Pridaná pocitová teplota;

vo východzom stave začiarknuté

# Oznamovať atmosferické informácie #

vo východzom stave nezačiarknuté

Ak je dostupné, môžete ho aktivovať:

* Pridaná hodnota vlhkosti;

Oznamuje vlhkosť v percentách.

vo východzom stave začiarknuté

* Pridaná hodnota viditeľnosti;

Oznamuje vzdialenosť viditelnosti v kilometroch alebo míľach.

vo východzom stave začiarknuté

* Pridaná hodnota atmosférického tlaku;

Indikuje atmosférický tlak v milibaroch alebo palcoch ortuti.

Ak je začiarknuté, povolte ďalšie začiarkavacie políčko, ktoré vám umožní uviesť tlak v milimetroch ortuti.

vo východzom stave začiarknuté

* Pridaný stav barometrického tlaku;

vo východzom stave začiarknuté

# Oznamovať atmosferické informácie #

Oznamuje čas východu a západu slnka.

vo východzom stave nezačiarknuté

# Používať  čiarku na oddelenie desatinných čísel#

Ak je povolené, používa čiarku ako oddeľovač desatinných čísel, inak sa ako oddeľovač používa bodka.

vo východzom stave nezačiarknuté

# Skontrolovať aktualizácie... #

Ak je aktivované, upozorní na dostupnú aktualizáciu doplnku.

vo východzom stave začiarknuté

Stlačte tlačidlo OK pre potvrdenie akcie alebo tlačidlo Zrušiť pre zrušenie akcie.

# pozn.: Vaše nastavenia budú uložené v súbore nazvanom: #
...
* "Weather.zipcodes": Zoznam miest s ich kódmi a definovaním oblasti.
...
* Pridané tlačidlo pre správu rozhrania API kľúča.
...
* Bolo pridaných 6 nových zvukových efektov;
...
* Boli opravené niektoré globálne premenné.
...
* Vylepšené oznamovanie aktuálneho času;
	* Pre niektoré mestá to nebolo oznamované správne.
...
* Weather Plus je teraz kompatibilný aj pre budúcu     wx verziu 4;
	* Poznámka: v súčasnosti  wx verzia 4.0.0b1 msw (phoenix) generuje nepríjemnú chybu pri použití vertikálnych šípok    v editovacích poliach:
...
* Priečinok Public links sa od 15. marca stane neaktívnym.
	* V tento deň sa stane priečinok Public štandardným priečinkom Dropboxu a
	* viac nebude použiteľný pre addon.
...
# Verzia 4.2 #
* Bolo pridaných 5 nových zvukových efektov;
	* Pre ich zfunkčnenie bude nevyhnutné ich aktualizovať v príslušnom nastavení doplnku.
 ...
 * Pridanénové reťazce do zoznamu správ o počasí.
...
# Verzia 4.0 #
* Boli aktualizované niektoré časti kódu a nahradené všetky inštrukcie eval().
...
* Pridaná pamäť pre správu a predpoveď počasia;
	...
	* To je preto, lebo správa sa nemení  v tejto dobe, čo zníži  frekvenciu volanní na rozhranie API,   je umožnené prehrávanie zvukových efektov.
...
* Bola pridaná kontrola aktualizácií;
...
* Bolo aktualizované okno  s dočasným kódom mesta;
	* Bol pridaný prvok"Hľadať" Ako v iných oknách doplnku Počasie:
...
`#`Zmeny v okně nastavenia doplnku Weather Plus:
...
`#`Zmeny v okně nastavenia Weather Plus:
...
# Verzia 2.7 #
* Opravený chybný  reťazec v súbore so zvukovými efektmi
...
* Pridaná možnosť ohlasovania informácíí o atmosfére;
	...
	* Zmeny pre okno s nastavením doplnku :
...
* Pridané tlačidlo podrobnosti o meste ";
	...
	* S geografickou koordináciou, a informáciou o nadmorskej výške.
...
# Verzia 2.6 #
* Funkcie tlačidiel pridať a odobrať boli optimalizované, načítanie zoznamu je oveľa rýchlejšie
...
* Pridané tri nové zvukové efekty, je nutné ich  aktualizovať z nastavenia doplnku
...
* Pridané nové reťazce pre ohlasovanie stavu počasia
...
# Verzia 2.4.4 #
* Pridané 2 nové reťazce pre ohlasovanie počasia
...
# Verzia 2.3 #
* Nastavovanie jednotiek stupňov je možné v jednom dialógu pre nastavenia
* Je možné pridávať, testovať,  vymazávať a pridávať do obľúbených kódy miest
...
* Aktualizovaný zdrojový kód doplnku, aby sa mohol prekladať
