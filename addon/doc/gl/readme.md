...
* Este complemento engade a temperatura local e o pronóstico do
    tempo actual a 24 horas e a predición ata 9 días adicionais
    para NVDA.
...
* Prema  NVDA + shift + control + alt + w para abrir o diálogo de configuración do Weather Plus.
* Prema NVDA + alt + w para coñecer a última actualización do reporte do tempo.
* 	Prema control + shift + w para alternar entre as escalas de temperatura Farenheit, Celsius e Kelvin.
...
F1: lugar de axuda, F2: selección da última pestana, F3: lista e caixa de edición, F4: controle a duración da previsión do tempo, F5: controis de volume.

Introduza unha cidade, WoeID ou seleccione unha da lista, se está dispoñible.

Nota: a tecla F5 só está dispoñible se os efectos de son están activados.

Despois de premer a opción Establecer e administrar as súas cidades, achará a continuación os seguintes botóns:

# Probar #

Proba a validez do WoeID da cidade introducido e pescuda o nome da cidade asignada, ou viceversa.

# Engadir #

Engadir a cidade actual á súa lista.

Este botón está activo se seleccionou unha cidade dende a lista e esta pasou a comprobación.
...
Este botón está activo se os efectos de son están instalados e activos, e se seleccionou unha cidade da lista e pasou a comprobación.

# Preestablecer #

Prestablecer a cidade como predeterminado, que será utilizado cando reinicie o complemento.

Este botón está activado se seleccionou unha cidade dende a lista e ésta non está preseleccionada e pasou a comprobación.
...
Este botón está activo se selecciona unha cidade insertada previamente na súa lista.

# Importar novas cidades... #

Permitir a incorporación á súa lista de novas cidades importándoas dende un arquivo con extensión *.zipcodes; pode seleccionar a cidade que desexa importar, activando a caixa de verificación.
...
Se os efectos de son están instalados e a caixa de verificación activada, dispoñibilizaranse a tecla f5 e o control de volume.

Tamén estará dispoñible unha caixa de verificación adicional:

* utilizar só efectos de tempo.

Pode cambiar o volume xeral ou o do último son escoitado e filtrar outros sons no seu entorno.
...
Se se habilita, permite escoitar só efectos de tempo como choiva, vento, raio, etc., filtrando todos os ambientais.
...
# Indica cun pitido mentres o boletín se está a cargar. #

Se está activado, escoitará un pitido para indicarlle o estado da busca en progreso mentres pescuda o informe do clima máis actualizado.
...
Se activado, tamén pode habilitar:
...
Se habilitado, permitir un adicional de caixa de verificación que permite indicar a presión en milímetros de mercurio;

Por defecto: sí.

* engadir estado da presión barométirca;
...
Se está habilitado, utiliza a coma coma separador decimal, doutro xeito utilízase o punto.

Por defecto: non.

# Verificar actualizacións #

Se está activa, notifica cando estea dispoñible unha actualización para o complemento.

Por defecto: si.

# Prema o botón de Aceptar para confirmar a acción, Cancelar para cancelala. #

Se modificou a lista de cidades, isto seralle lembrado se preme o botón cancelar a fin de que aínda poida salvar as modificacións.

# Nota: a súa configuración será gardada nos seguintes arquivos dentro da carpeta %appdata%\NVDA\ ou *rutaNVDA*\user-config en versións portables: #

* "Weather.ini": configuración de inicio do Weather Plus.
* "Weather.volumes": niveis de volume de audio personalizado, calqueira que sexa o volume xeral do sistema.
* "Weather.zipcodes": lista de cidades cos seus códigos postais e definicións.
...
* Prácticamente todas as características da versión anterior 4.8 están de volta e mantén o botón "Renomear".
* Agora compatible tamén con Python 3.

# Versión 5.0.1 #
* Corrixido erro que causaba inconsistencia entre os efectos de son da zona horaria e os reproducidos.
* Axustado o número de días de previsión de 9 a 6 no readme.
* Arranxado erro polo que se devolvía unha cadea vacía se a velocidade do vento en mph era 0.

# Versión 5.0 #
* Weather Plus utiliza agora a API de APIXU, na miña opinión mellor que a anterior.
...
* Engadid botón para administrar a súa clave da API.

# Versión 4.8 #
`#`Trocos na xanela de Configuración de WeatherPlus:
...
# Versión 4.7.7 #
* Eliminada a notificación innecesaria de descarga completada ao actualizar WeatherPlus.
...
# Versión 4.7.5 #
* MenuItem.GetLabel() está desaconsellado e cambiouse por MenuItem.GetItemLabelText().
...
# Versión 4.6.8 #
* Actualizadas as traducións portuguesa e brasileira (grazas a Alberto Mendonça).
...
# Versión 4.6.2 #
* Arranxado erro: Despois de “Verificar actualizacións”, o menú “Establecer unha cidade temporal…” foi habilitado aínda que non estea dispoñible ningunha lista de cidades.
* Arranxado erro; imposible configurar WP se weather.ini non estaba creadoaínda.
...
* engadidas sete novas caixas de verificación;
...
* Indicates the atmospheric pressure in millimeters of mercury (mmHg);
	* Only if the degrees are set in Fahrenheit.
...
# Versión 4.4 #
* Arranxado un erro na lectura da cadea da nova versión durante unha conexión expirada.
...
# Versión 3.7 #
* engadida a posibilidade de deshabilitar a conversión a metros por segundo da velocidade do vento.
* Engadida a posibilidade de usar libras por polgada cadrada coma unidade de presión.
* Arranxados dous erros.
...
* Actualización da función de reprodución de efectos de son, agora utilízase MP3, cun aforro considerable no tempo de descarga e espazo en disco, grazas á redución do tamaño dos arquivos comprimidos.
* Engadíronse 55 novos efectos de son, será necesario actualizar dende a configuración do complemento.

# Información de interese #

A versión 3.1 é a base para a tradución do add-on. Polo tanto, non se incluíron os cambios para versións anteriores. Desculpe as molestias que se puideran ocasionar.
